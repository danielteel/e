...
const char* stringToEncrypt = "DanTeel";
...
uint8_t* frame(uint8_t* data, uint16_t length, uint16_t& framedLength) {
...
uint8_t* deframe(uint8_t* data, uint16_t framedLength, uint16_t& dataLength) {
...
void encrypt(uint8_t* buffer, uint16_t bufferLength, const char* keyString) {
    uint8_t key[32];
    char tempHex[] = { 0,0,0 };
    ...
    for (uint8_t k = 0; k < 32; k++) {
        for (uint32_t i = 0; i < bufferLength - 7; i++) {
...
void decrypt(uint8_t* buffer, uint16_t bufferLength, const char* keyString) {
    uint8_t key[32];
    char tempHex[] = { 0,0,0 };
    for (uint8_t i = 0; i < 64; i += 2) {
        tempHex[0] = keyString[i];
    ...
    for (uint8_t k = 31; k >= 0 && k!=0xFF; k--) {
        for (uint32_t i = bufferLength - 8; i >= 0 && i!=0xFFFFFFFF; i--) {
            buffer[i + 7] ^= leftRotate8(key[k], 6 + buffer[i + 6] % 2);
            buffer[i + 6] ^= leftRotate8(key[k], 5 + buffer[i + 5] % 2);
            buffer[i + 5] ^= leftRotate8(key[k], 4 + buffer[i + 4] % 2);
            buffer[i + 4] ^= leftRotate8(key[k], 3 + buffer[i + 3] % 2);
...
void doThang()
{
    std::cout << "Original:  size=" << strlen(stringToEncrypt) + 1 << "  " << stringToEncrypt << "\n";

    uint16_t framedLength = 0;
    uint8_t* framed = frame((uint8_t*)stringToEncrypt, strlen(stringToEncrypt)+1, framedLength);

    std::cout << "Framed:    size=" << std::to_string(framedLength) << " ";
    for (unsigned int i = 0; i < framedLength; i++) {
        int l = strlen(std::to_string(framed[i]).c_str());
        for (int i = 0; i < 3 - l; i++) {
            std::cout << "0";
        }
        std::cout << std::to_string(framed[i]);
    }

    encrypt(framed, framedLength, keyString);
    std::cout << "\nEncrypted: size=" << std::to_string(framedLength) << " ";
    for (unsigned int i = 0; i < framedLength; i++) {
        int l = strlen(std::to_string(framed[i]).c_str());
        for (int i = 0; i < 3 - l; i++) {
            std::cout << "0";
    ...
    decrypt(framed, framedLength, keyString);
    std::cout << "\nDecrypted: size="<<std::to_string(framedLength)<<" ";
    for (unsigned int i = 0; i < framedLength; i++) {
        int l = strlen(std::to_string(framed[i]).c_str());
        for (int i = 0; i < 3 - l; i++) {
            std::cout << "0";
    ...
    uint16_t deframedLength = 0;
    uint8_t* deframed = deframe(framed, framedLength, deframedLength);

    std::cout << "Deframed:  size=" << std::to_string(deframedLength) << "  " << deframed << "\n";
